	...
	$(".sq_resume").click(function(){
		...
		$.post(wapurl+"/index.php?c=ajax&a=indexajaxresume",{show_job:1},function(data){
			...
			if(status == '7'){
				...
				var msg='会员已到期，继续操作将会扣除'+integral+'元，是否继续？';
				layer.open({
					content: msg,
					btn: ['继续', '取消'],
					shadeClose: false,
...
function _addjob(num,integral_job,have, gourl,checkType)
{
	if(num==1){
		location.href=gourl;
	}else if(num==2){
		if(integral_job>0){
			var msg='套餐已用完，继续操作将会扣除'+integral_job+'元，是否继续？';
			layer.open({
				content: msg,
				btn: ['继续', '取消'],
				shadeClose: false,
...
function jobadd_url(num,integral_job,type,have,id){
	var checkType = '';
	var gourl='';
	if(type=="part"){
		if(id){
	...
	if(checkType == ''){
		_addjob(num,integral_job,have, gourl);
	}
	else{
			...
			function(data){
				...
				if(data.status == -1){
					layermsg(data.msg);
 					...
 					_addjob(num,integral_job,have, gourl,checkType);
				}
			}
		);
	}
					...
					$.post(url,{show_job:'1'},function(data){
						...
						if(status == 7){
							...
							layermsg('您还没有正在招聘的职位，请先发布职位吧！');return false;
 							...
 							layer.open({
								content:"会员已到期，邀请面试将扣除"+integral+"元，是否继续？",
								btn: ['确认', '取消'],
								shadeClose: false,
								yes: function(){
