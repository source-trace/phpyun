	...
	$.post(url,{eid:eid},function(data){
		...
		if(status==1){
			var j = layer.confirm('发布职位后才可以下载简历',
				{btn : ['前去发布','继续浏览']},
				function(){
				  window.location.href=weburl+"/member/index.php?c=jobadd";window.event.returnValue = false;return false;
				},
				function(){
				  layer.close(j);
			...
			if(data.usertype=='2'){
				var k = layer.confirm(data.msg, function(){
					...
					//付费操作
					buyResume(eid);		
					//down_integral(eid,data.uid,downurl);
				});
			}else if(data.usertype=='3'){
				layer.confirm(data.msg, function(){
					buyLtResume(eid);		
					//lt_down_integral(eid,data.uid,downurl);
				});
			} 
		}else if(status==3){ 
			if(todown){ 
...
function showpacklist(){
	var i=layer.load('执行中，请稍候...',0);
	$.post(weburl+"/index.php?m=ajax&c=getPack",{rand:Math.random()},function(data){
		layer.close(i);
...
function settemplate(msg,url){ 
	layer.confirm(msg, function(){
		...
		$.get(url,function(data){
			...
			var data=eval('('+data+')');
			if(data.st=='8'){
				layer.msg(data.msg, Number(data.tm), Number(data.st),function(){layer.closeAll('iframe');});return false;	 
			}else{
...
function talent_pool(uid,eid,url){
	...
	$.post(url,{eid:eid,uid:uid,remark:remark},function(data){
		if(data=='0'){
			...
			layer.msg('加入成功！',2,9,function(){location.reload();});
		}else if(data=='2'){
			layer.msg('该简历已加入到人才库！',2,8,function(){layer.close(layer.index);});
		}else{
