				...
				if(data.status == -1){
					...
					$.post(weburl+"/index.php?m=ajax&c=indexajaxresume",{show_job:1,jobid:jobid,jobtype:jobtype},function(data){
						...
						if(status == 7){
							...
							if(buypack==1){
								var msg="会员已到期，继续邀请将消费"+integral+"元，是否继续？<br>您还可以<a href=\"javascript:;\" onClick=\"showpacklist()\" style='color:red;cursor:pointer;'>购买增值包</a>";
							}else{
								var msg="会员已到期，继续邀请将消费"+integral+"元，是否继续？<br>您还可以<a href='"+weburl+"/member/index.php?c=right' style='color:red;cursor:pointer;'>购买会员</a>";
							}
							// 购买邀请面试弹出框
							...
							if(buypack==1){
								...
								var msg="你的等级特权已经用完，继续邀请将消费"+integral+"元，是否继续？您还可以<a href='"+weburl+"/member/index.php?c=right&act=added' style='color:red;cursor:pointer;'>购买增值包</a>";
							}
							//购买邀请面试弹出框
							layer.confirm(msg,function(){
								var height="300px";
